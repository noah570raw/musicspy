...
function hasConfiguredDatabaseUrl(env = process.env) {
  return Boolean(String(env.DATABASE_URL || env.POSTGRES_URL || env.POSTGRESQL_URL || "").trim());
}

function isStrictDatabaseRequired(env = process.env) {
  return String(env.MUSICSPY_REQUIRE_DATABASE || "").toLowerCase() === "true";
}
...
function createUserStorePersistence({ env = process.env, fsImpl = fs, logger = console } = {}) {
  ...
  const configuredDatabaseUrl = hasConfiguredDatabaseUrl(env);
  if (!configuredDatabaseUrl && isStrictDatabaseRequired(env)) {
...
module.exports = {
  ...
  isRenderEnvironment: postgresStore.isRenderEnvironment,
  isStrictDatabaseRequired,
  shouldUsePostgres: postgresStore.shouldUsePostgres,
  friendshipKey,
