...
const EMPTY_USERS_STORE = Object.freeze({ users: [], friendships: [], friendRequests: [], directMessages: [], globalChatMessages: [] });
...
async function loadUserStore() {
  await initializeDatabase();
  const [{ rows: userRows }, { rows: inventoryRows }, { rows: friendRows }, { rows: messageRows }, { rows: globalRows }] = await Promise.all([
    ...
    pool.query("SELECT * FROM global_chat_messages ORDER BY created_at ASC LIMIT 80")
  ]);
  ...
  const globalChatMessages = globalRows.map((row) => ({
    id: row.id,
  ...
  return { users, friendships, friendRequests, directMessages, globalChatMessages };
}
...
async function saveUserStore(store = EMPTY_USERS_STORE) {
  ...
  try {
    ...
    for (const message of asArray(store.globalChatMessages).slice(-80)) {
      await client.query(`
        INSERT INTO global_chat_messages (id, user_payload, text, created_at)
        VALUES ($1, $2::jsonb, $3, COALESCE($4, NOW()))
        ON CONFLICT (id) DO UPDATE SET user_payload = EXCLUDED.user_payload, text = EXCLUDED.text
      `, [message.id, JSON.stringify(message.user || {}), message.text, message.createdAt || null]);
    }
