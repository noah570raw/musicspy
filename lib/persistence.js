...
function createUserStorePersistence({ env = process.env, fsImpl = fs, logger = console } = {}) {
  ...
  if (renderEnvironment && !String(env.DATABASE_URL || env.POSTGRES_URL || env.POSTGRESQL_URL || "").trim()) {
    throw new Error("Render production persistence requires DATABASE_URL / Render PostgreSQL; refusing ephemeral file storage");
  }
