...
function createUserStorePersistence({ env = process.env, fsImpl = fs, logger = console } = {}) {
  ...
  const hasDatabaseUrl = String(env.DATABASE_URL || env.POSTGRES_URL || env.POSTGRESQL_URL || "").trim();
  ...
  const usingRenderPersistentDisk = renderEnvironment && (String(env.RENDER_DISK_MOUNT_PATH || "").trim() || dataDir.startsWith(`${RENDER_PERSISTENT_DIR}${path.sep}`));

  if (renderEnvironment && !hasDatabaseUrl) {
    if (usingRenderPersistentDisk) {
      logger.warn?.(`[db] Render PostgreSQL DATABASE_URL is not configured; using persistent Render disk at ${dataDir} instead of ephemeral runtime storage`);
    } else {
      logger.warn?.(`[db] Render PostgreSQL DATABASE_URL is not configured and no Render disk was detected; using ${dataDir}. Attach Render PostgreSQL or a persistent disk before storing production users.`);
    }
  }
