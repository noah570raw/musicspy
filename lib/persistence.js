...
function isRenderEnvironment(env = process.env) {
  return Boolean(String(env.RENDER || env.RENDER_SERVICE_ID || env.RENDER_SERVICE_NAME || "").trim());
}

function shouldRequireDatabase(env = process.env) {
  return isProductionRuntime(env) || isRenderEnvironment(env);
}

function isLocalDevelopment(env = process.env) {
  return !shouldRequireDatabase(env);
}
...
function createPostgresUserStorePersistence({ env = process.env, fsImpl = fs, logger = console, pgModule } = {}) {
  ...
  const pool = new Pool({
    ...
    // Render PostgreSQL requires TLS for managed database URLs. Render uses an
    // internal certificate chain, so rejectUnauthorized=false keeps Node pg
    // compatible with both Render Internal Database URLs and most local tunnels.
    ...
    allowExitOnIdle: !shouldRequireDatabase(env)
  });

  pool.on("error", (error) => {
    logger.error?.("[db:postgres] idle client error; pool will reconnect automatically on the next query", error);
  });
  const dataDir = resolveDataDir(env, fsImpl);
  const legacyUsersFile = path.join(DEFAULT_DATA_DIR, "users.json");
  ...
  async function init() {
    ...
    logger.log?.("[db:postgres] connection established; initializing schema");
    await query(`
...
function logMissingDatabaseWarning({ env = process.env, logger = console } = {}) {
  const runtime = isProductionRuntime(env) ? "production" : "development";
  const render = isRenderEnvironment(env) ? "Render" : "non-Render";
  logger.warn?.("[db] DATABASE_URL is not configured; PostgreSQL persistence is unavailable.");
  logger.warn?.("[db] Missing environment variable: DATABASE_URL");
  logger.warn?.("[db] Create a PostgreSQL database in Render");
  logger.warn?.("[db] Copy Internal Database URL");
  logger.warn?.("[db] Add DATABASE_URL environment variable to Web Service");
  logger.warn?.(`[db] Runtime=${runtime}; platform=${render}; ephemeral JSON persistence is disabled for production/Render.`);
}

function createUnavailableProductionPersistence({ env = process.env, logger = console } = {}) {
  logMissingDatabaseWarning({ env, logger });
  return {
    kind: "unconfigured-postgres",
    dataDir: null,
    usersFile: "DATABASE_URL is not configured",
    legacyUsersFile: path.join(DEFAULT_DATA_DIR, "users.json"),
    init: () => undefined,
    read: () => normalizeUserStore(EMPTY_USERS_STORE),
    write: async () => {
      throw new Error("DATABASE_URL is required before persistent user data can be written in production/Render.");
    },
    close: () => undefined
  };
}
...
function createUserStorePersistence(options = {}) {
  ...
  if (postgres) {
    logger.log?.("[db] persistence mode: PostgreSQL via DATABASE_URL");
    return postgres;
  }

  if (shouldRequireDatabase(env) && !String(env.MUSICSPY_ALLOW_FILE_STORE || "").trim()) {
    return createUnavailableProductionPersistence({ ...options, env, logger });
  }

  logger.warn?.("[db:file] DATABASE_URL is not configured; using local JSON persistence for localhost development/test only.");
  logger.warn?.("[db:file] Do not use this mode on Render because redeploys can wipe filesystem data.");
...
module.exports = {
  ...
  createUnavailableProductionPersistence,
