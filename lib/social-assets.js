...
function drawSpySilhouette(canvas) {
  const cx = 842;
  const base = 584;
  glow(canvas, cx + 50, 286, 210, 320, "#be123c", 0.38);
  glow(canvas, cx - 68, 294, 230, 300, "#2563eb", 0.28);

  fillEllipse(canvas, cx + 10, 502, 135, 170, "#03040c", 0.98);
  fillPolygon(canvas, [[cx - 158, base], [cx - 82, 354], [cx - 8, 444], [cx + 52, 354], [cx + 154, base]], "#02030a", 0.99);
  fillPolygon(canvas, [[cx - 98, 364], [cx - 18, 438], [cx - 60, 574]], "#101827", 0.52);
  fillPolygon(canvas, [[cx + 62, 364], [cx + 8, 438], [cx + 76, 574]], "#111827", 0.46);

  fillEllipse(canvas, cx, 264, 76, 92, "#02030a", 0.99);
  roundedRect(canvas, cx - 124, 190, 248, 42, 21, "#02030a", 0.99);
  fillPolygon(canvas, [[cx - 70, 194], [cx - 36, 124], [cx + 62, 124], [cx + 96, 194]], "#02030a", 0.99);

  strokeLine(canvas, cx - 90, 232, cx + 100, 232, "#e879f9", 2, 0.42);
  strokeLine(canvas, cx - 40, 286, cx - 8, 286, "#f8fafc", 2, 0.5);
  strokeLine(canvas, cx + 16, 284, cx + 46, 284, "#f8fafc", 2, 0.45);
  strokeLine(canvas, cx - 108, 370, cx - 154, 584, "#60a5fa", 3, 0.16);
  strokeLine(canvas, cx + 82, 370, cx + 158, 584, "#f472b6", 3, 0.14);
}
...
function renderSocialPreview() {
  ...
  drawReactionBubble(canvas, 1032, 160, 34, "#f472b6");
  drawReactionBubble(canvas, 1014, 458, 28, "#8b5cf6");
  drawReactionBubble(canvas, 642, 118, 24, "#60a5fa");

  drawStrokeText(canvas, "MUSIC SPY", 78, 164, 12, "#0b1022", 14, 0.82);
  drawStrokeText(canvas, "MUSIC SPY", 72, 156, 12, "#ffffff", 11, 1);
  strokeLine(canvas, 78, 256, 456, 256, "#e879f9", 3, 0.5);
  strokeLine(canvas, 78, 266, 334, 266, "#60a5fa", 2, 0.35);

  drawStrokeText(canvas, "НАЙДИ ШПИОНА", 78, 334, 6, "#f8fafc", 5, 0.94);
  drawStrokeText(canvas, "СРЕДИ ДРУЗЕЙ", 78, 392, 6, "#c4b5fd", 5, 0.88);
