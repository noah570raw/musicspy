...
WHERE user_id IN (SELECT id FROM jsonb_to_recordset(${dollarQuote(JSON.stringify(users))}::jsonb) AS x(id text, data jsonb))
  AND id NOT IN (SELECT id FROM jsonb_to_recordset(${dollarQuote(JSON.stringify(sessions))}::jsonb) AS x(id text, user_id text, data jsonb));
DELETE FROM match_history
WHERE user_id IN (SELECT id FROM jsonb_to_recordset(${dollarQuote(JSON.stringify(users))}::jsonb) AS x(id text, data jsonb))
  AND id NOT IN (SELECT id FROM jsonb_to_recordset(${dollarQuote(JSON.stringify(matchHistory))}::jsonb) AS x(id text, user_id text, lobby_id text, data jsonb));
DELETE FROM player_progression
WHERE user_id IN (SELECT id FROM jsonb_to_recordset(${dollarQuote(JSON.stringify(users))}::jsonb) AS x(id text, data jsonb));
DELETE FROM direct_messages
WHERE id NOT IN (SELECT id FROM jsonb_to_recordset(${dollarQuote(JSON.stringify(directMessages))}::jsonb) AS x(id text, data jsonb));
DELETE FROM friend_requests
WHERE id NOT IN (SELECT id FROM jsonb_to_recordset(${dollarQuote(JSON.stringify(friendRequests))}::jsonb) AS x(id text, data jsonb));
DELETE FROM friendships
WHERE id NOT IN (SELECT id FROM jsonb_to_recordset(${dollarQuote(JSON.stringify(friendships))}::jsonb) AS x(id text, data jsonb));
...
ON CONFLICT (id) DO UPDATE SET data = EXCLUDED.data, access_expires_at = EXCLUDED.access_expires_at, refresh_expires_at = EXCLUDED.refresh_expires_at, updated_at = EXCLUDED.updated_at;
...
ON CONFLICT (id) DO UPDATE SET user_a_id = EXCLUDED.user_a_id, user_b_id = EXCLUDED.user_b_id, data = EXCLUDED.data;
...
ON CONFLICT (id) DO UPDATE SET sender_id = EXCLUDED.sender_id, receiver_id = EXCLUDED.receiver_id, status = EXCLUDED.status, data = EXCLUDED.data, responded_at = EXCLUDED.responded_at;
...
ON CONFLICT (id) DO UPDATE SET conversation_id = EXCLUDED.conversation_id, sender_id = EXCLUDED.sender_id, receiver_id = EXCLUDED.receiver_id, status = EXCLUDED.status, data = EXCLUDED.data;
...
ON CONFLICT (id) DO UPDATE SET user_id = EXCLUDED.user_id, lobby_id = EXCLUDED.lobby_id, data = EXCLUDED.data;
