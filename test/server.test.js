...
const { countReactions, getActiveTurnOrder, removePlayerFromLobby } = require("../server");
...
test("removePlayerFromLobby clears a kicked player from game state", () => {
  const lobby = {
    ...
    currentTrackReactions: { host: "🔥", kicked: "💀" },
    currentTurnIndex: 2
  };
  ...
  assert.deepEqual(lobby.currentTrackReactions, { host: "🔥" });
  assert.equal(lobby.currentTurnIndex, 1);
});

test("countReactions totals only supported track reactions", () => {
  assert.deepEqual(countReactions({
    host: "🔥",
    player1: "🔥",
    player2: "🕵️",
    player3: "not-supported"
  }), {
    "🔥": 2,
    "🕵️": 1
  });
